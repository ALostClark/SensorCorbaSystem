...
public class _CentralServerStub
	extends org.omg.CORBA.portable.ObjectImpl
	implements clark.corba.CentralServer
{
	/** Serial version UID. */
	...
	public final static java.lang.Class _opsClass = clark.corba.CentralServerOperations.class;
		...
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					...
					java.lang.String tmpResult8 = location;
_os.write_string( tmpResult8 );
		...
		else
		{
			...
			if( _so == null )
				continue;
			CentralServerOperations _localServant = (CentralServerOperations)_so.servant;
			try
			{
				_localServant.register(location);
		...
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					...
					java.lang.String tmpResult9 = control_station;
_os.write_string( tmpResult9 );
		...
		else
		{
			...
			try
			{
				_localServant.raise_alarm(control_station);
				if ( _so instanceof org.omg.CORBA.portable.ServantObjectExt) 
					((org.omg.CORBA.portable.ServantObjectExt)_so).normalCompletion();
				return;
			}
...
_os.write_string( tmpResult10 );
		...
		else
		{
			...
			try
			{
				_localServant.unregister(location);
				if ( _so instanceof org.omg.CORBA.portable.ServantObjectExt) 
					((org.omg.CORBA.portable.ServantObjectExt)_so).normalCompletion();
				return;
			}
